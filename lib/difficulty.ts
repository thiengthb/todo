...
export function computeDifficultyHints(
  rated: { title: string; emotion: string | null }[],
): DifficultyHints {
  // gom theo keyword: đếm số lần hard / love / tổng
  const byWord = new Map<
    string,
    { hard: number; love: number; total: number }
  >();
  let samples = 0;
  ...
  for (const t of rated) {
    ...
    for (const w of seen) {
      const c = byWord.get(w) ?? { hard: 0, love: 0, total: 0 };
      c.total += 1;
