  ...
  return (
    <div className="mt-4 flex items-start gap-2.5 rounded-lg border border-amber-300 bg-amber-50 p-3 dark:border-amber-900 dark:bg-amber-950/40">
      ...
      <p className="text-xs leading-relaxed text-amber-700 dark:text-amber-300">
        Chuỗi <span className="font-medium tabular-nums">{current}</span> ngày
        vẫn đang giữ — hoàn thành <span className="font-medium">1 việc</span>{" "}
        hôm nay là nối tiếp. Lỡ một ngày không sao, đừng để lỡ hai.
      </p>
    </div>
  );
}
