  ...
  return (
    <section className="space-y-3">
      <h2 className="text-sm font-medium">
        Lịch cứng theo tuần{' '}
        <span className="font-normal text-muted-foreground">({commitments.length})</span>
      </h2>
  ...
  const header = (
    <div className="flex items-center justify-between gap-2">
      <h2 className="flex items-center gap-1.5 text-sm font-medium">
        ...
        Khung giờ tập trung{' '}
        {softBlocks.length > 0 && (
          <span className="font-normal text-muted-foreground">({softBlocks.length})</span>
        )}
        <InfoHint label="Khung giờ tập trung là gì?">
          Khung giờ lặp theo tuần bạn <strong className="font-medium text-foreground">muốn</strong>{' '}
          dành cho một việc (vd 20–22h học). Khác lịch cứng: nó{' '}
          <strong className="font-medium text-foreground">dời được</strong> và không bị trừ khỏi quỹ
          giờ rảnh — chỉ gợi ý cho AI biết bạn ưu tiên khung nào.
        </InfoHint>
      </h2>
...
function ScheduleForm({
  ...
  const editingCommitment = editing.kind === 'edit-commitment' ? editing.data : null;
  ...
  const [showTerm, setShowTerm] = useState(!!(validFrom || validUntil || weekParity));
  // event
  const [date, setDate] = useState(
    editingEvent?.date ??
      (editing.kind === 'new-event' || editing.kind === 'new-from-grid' ? editing.date : weekStart),
  );
  ...
  const [evStart, setEvStart] = useState(
    editingEvent?.startTime ?? (editing.kind === 'new-from-grid' ? editing.start : '09:00'),
  );
  const [evEnd, setEvEnd] = useState(
    editingEvent?.endTime ?? (editing.kind === 'new-from-grid' ? editing.end : '11:00'),
  );
    ...
    <>
      ...
      <div className="space-y-4">
        ...
        {!(type === 'event' && eventMode === 'off') && (
          <Field
            label={
              type === 'commitment' ? 'Tên lịch' : type === 'soft' ? 'Tên khung giờ' : 'Tên sự kiện'
            }
          >
            <Input
              value={title}
        ...
        {type === 'commitment' || type === 'soft' ? (
          <>
            <Field label="Thứ trong tuần">
              <div className="flex flex-wrap gap-1">
                {WEEK_ORDER.map((dw) => (
                  <SegBtn key={dw} active={dayOfWeek === dw} onClick={() => setDayOfWeek(dw)}>
                    {weekdayShortVN(dw)}
                  </SegBtn>
                ))}
              </div>
            ...
            {showTerm ? (
              <div className="space-y-3 rounded-md border border-border/70 bg-muted/30 p-3">
                ...
                <div className="flex items-center gap-1">
                  <SegBtn active={!weekParity} onClick={() => setWeekParity(null)}>
                    Mọi tuần
                  </SegBtn>
                  <SegBtn active={weekParity === 'odd'} onClick={() => setWeekParity('odd')}>
                    Tuần lẻ
                  </SegBtn>
                  <SegBtn active={weekParity === 'even'} onClick={() => setWeekParity('even')}>
                    Tuần chẵn
                  </SegBtn>
                </div>
              </div>
          ...
          <>
            ...
            {eventMode === 'off' && (
              <p className="text-xs text-muted-foreground">
                Đánh dấu nghỉ cả ngày này — mọi lịch cứng hôm đó sẽ được bỏ qua khi tính quỹ giờ
                rảnh.
              </p>
            )}
            {eventMode === 'timed' && (
              <TimeRange start={evStart} end={evEnd} onStart={setEvStart} onEnd={setEvEnd} />
            )}
          </>
        )}
          ...
          <Field label="Phân loại">
            <div className="flex gap-1">
              {SCHEDULE_KINDS.map((k) => (
                <SegBtn key={k.value} active={kind === k.value} onClick={() => setKind(k.value)}>
                  {k.label}
                </SegBtn>
              ))}
            </div>
...
function TimeRange({
  ...
  return (
    <div className="flex items-center gap-2">
      <TimePicker value={start} onChange={onStart} className="flex-1" ariaLabel="Giờ bắt đầu" />
      <span className="text-muted-foreground">→</span>
      <TimePicker value={end} onChange={onEnd} className="flex-1" ariaLabel="Giờ kết thúc" />
    </div>
  );
}
